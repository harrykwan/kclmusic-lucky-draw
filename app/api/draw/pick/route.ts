...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!entries.length) return NextResponse.json({ error: "No entries yet" }, { status: 404 });
    return NextResponse.json({ winner: entries[Math.floor(Math.random() * entries.length)] });
